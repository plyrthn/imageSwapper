...
def match_template(template_files):
    ...
    try:
        ...
        if clean_size_str.count('x') == 1:
            ...
            logging.debug(f"Attempting to use template: {template_path} with size: {width}x{height}")
            return {'path': template_path, 'size': (width, height), 'format': 'DDS'}
        else:
            logging.error(f"Unexpected size format for template {template_path}: {clean_size_str}")
...
def process_images(input_folder, template_folder, output_folder, do_not_duplicate=False):
    ...
    with ProcessPoolExecutor() as executor:
        ...
        if do_not_duplicate:
            ...
            input_files_iterator = itertools.cycle(input_files)  # Prepare for potentially infinite cycling

        try:
            while template_files:
                # If not duplicating, shuffle the input_files list each time it's fully iterated
                if do_not_duplicate and future_to_file and all(future.done() for future in future_to_file):
                ...
                future = executor.submit(resize_and_convert_image, input_file, output_path_dds, template_info['size'])
...
def resize_and_convert_image(input_file, output_path_dds, size):
    try:
        subprocess.run(['magick', 'convert', input_file, '-resize', f"{size[0]}x{size[1]}!", output_path_dds],
